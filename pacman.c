...
unsigned int total_time = 0; // total time during AI search
int info[3] = {0,0,0}; // summary info: max_depth, total_generated_node, total_expanded_node
...
void ExitProgram(const char *message) {
	...
	printf("Propagation = ");
	if( propagation == 0 ) printf("Max\n");
	else printf("Avg\n");
	
	printf("Budget = %d\n", budget);
	printf("MaxDepth = %d\n", info[0]); 
	printf("TotalGenerated = %d\n", info[1]); 
	printf("TotalExpanded = %d\n", info[2]);
	printf("Time = %.2f seconds\n", total_time/1000.0);
	printf("Expanded/Second = %.2f\n", 1000.0*info[2]/total_time); 
	printf("Score = %d\n", Points);
...
void MainLoop() {
    ...
    do {
	    ...
	    if(ai_run){
			...
			ftime(&t_current);
			total_time += (int)(t_current.millitm - t_start.millitm);
